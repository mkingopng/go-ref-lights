...
// --------------- Test Left/Center/Right (minimal) ---------------

func TestLeftView(t *testing.T) {
	resetPageControllerGlobals()
	router := setupTestRouter(t)
	router.GET("/left", Left)

	t.Run("No meet => redirect /meets", func(t *testing.T) {
		req, _ := http.NewRequest("GET", "/left", nil)
		w := performRequest(router, req)
		assert.Equal(t, http.StatusFound, w.Code)
		assert.Equal(t, "/meets", w.Result().Header.Get("Location"))
	})

	t.Run("Success => 200", func(t *testing.T) {
		ck := SetSession(router, "/setSessionLeft", map[string]interface{}{
			"meetName": "myMeet",
		})
		req, _ := http.NewRequest("GET", "/left", nil)
		req.AddCookie(ck)
		...
		w := performRequest(router, req)
		assert.Equal(t, http.StatusOK, w.Code)
		assert.Contains(t, w.Body.String(), "Left ref view for myMeet")
	})
}

func TestCenterView(t *testing.T) {
	resetPageControllerGlobals()
	router := setupTestRouter(t)
	...
	t.Run("Success => 200", func(t *testing.T) {
		ck := SetSession(router, "/setSessionCenter", map[string]interface{}{
			"meetName": "anotherMeet",
		})
		req, _ := http.NewRequest("GET", "/center", nil)
		req.AddCookie(ck)
		...
		w := performRequest(router, req)
		assert.Equal(t, http.StatusOK, w.Code)
		assert.Contains(t, w.Body.String(), "Center ref view for anotherMeet")
	})
}
...
func TestRightView(t *testing.T) {
	...
	t.Run("No meet => redirect /meets", func(t *testing.T) {
		req, _ := http.NewRequest("GET", "/right", nil)
		w := performRequest(router, req)
		assert.Equal(t, http.StatusFound, w.Code)
		assert.Equal(t, "/meets", w.Result().Header.Get("Location"))
	})
	...
	t.Run("Success => 200", func(t *testing.T) {
		ck := SetSession(router, "/setSessionRight", map[string]interface{}{
			"meetName": "CoolMeet",
		})
		req, _ := http.NewRequest("GET", "/right", nil)
		req.AddCookie(ck)
		...
		w := performRequest(router, req)
		assert.Equal(t, http.StatusOK, w.Code)
		assert.Contains(t, w.Body.String(), "Right ref view for CoolMeet")
	})
}
